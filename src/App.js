...
/**
 * 移动端待办事项管理应用 (v10.2 - 标题美化版)
 * - 移除 Header 左侧图标
 * - 标题采用杂志级双层排版 (Tiny Uppercase + Serif Bold)
 */
...
export default function App() {
  ...
  const processTagMutex = (currentTags, tagToAdd) => {
    if (tagToAdd === "forgettable") {
      const tempTags = currentTags.filter(
                        ...
                        <div className="absolute top-12 right-0 bg-white/95 backdrop-blur-xl rounded-xl shadow-2xl border border-white/60 z-50 w-44 p-2 animate-in fade-in zoom-in-95 duration-200 flex flex-col gap-1 max-h-80 overflow-y-auto">
                          ...
                          {PRESET_TAGS.map((tag) => (
                            <FilterOption
                              ...
                              color={
                                tag.color === PRIORITY_COLORS.URGENT ||
                                tag.color === PRIORITY_COLORS.IMPORTANT
                                  ? themeColor
                                  : tag.color
                              }
                              label={tag.label}
              ...
              <div className="p-6 max-h-[60vh] overflow-y-auto">
                <div className="mb-8">
                  ...
                  <div className="mt-4 pt-4 border-t border-gray-100/50">
                    <p className="text-[10px] font-bold text-gray-400 uppercase mb-2 tracking-[0.2em]">
                      自定义标签
                    </p>
